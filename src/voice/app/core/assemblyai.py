...
REPO_ROOT = VOICE_ROOT.parents[1] if VOICE_ROOT.parent.name == "src" else VOICE_ROOT
ENV_FILE = VOICE_ROOT / ".env"
AGENT_DIR = VOICE_ROOT / "app" / "agents"
ROOT = VOICE_ROOT


def env_files() -> list[Path]:
    """`src/voice/.env` wins over the monorepo `.env`; process env wins over both."""
    paths = [VOICE_ROOT / ".env", REPO_ROOT / ".env"]
...
def load_env(path: Path | None = None) -> None:
    ...
    files = [path] if path is not None else env_files()
...
def save_env(key: str, value: str, path: Path | None = None) -> bool:
    """Save a generated setting to .env when the filesystem is writable."""
    if path is None:
        path = REPO_ROOT / ".env" if (REPO_ROOT / ".env").exists() else ENV_FILE
