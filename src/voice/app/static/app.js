...
let turnCounter = 0;
const ALLOWED_SPEAKERS = new Set(['patient', 'caregiver', 'staff', 'agent', 'unknown'])
const SPEAKER_ALIASES = { you: 'patient', user: 'patient', caller: 'patient', assistant: 'agent', aira: 'agent' }

function sanitizeSpeaker(speaker) {
  if (typeof speaker !== 'string') return 'unknown'
  const key = speaker.trim().toLowerCase()
  if (ALLOWED_SPEAKERS.has(key)) return key
...
function addConversationTurn(speaker, text) {
  const cleanText = sanitizeTurnText(text)
  if (!cleanText) return
...
async function submitHandoff(sessionId) {
  const validTurns = conversationTurns.filter(
    (turn) => ALLOWED_SPEAKERS.has(turn.speaker) && sanitizeTurnText(turn.text)
  )
  if (!validTurns.length) {
    console.log('ROLE 1 HANDOFF skipped: no valid transcript turns')
    return
  }
  try {
    ...
    console.log('ROLE 1 -> ROLE 2/3 HANDOFF:')
    console.log(JSON.stringify(body, null, 2))
    const caseId = body?.intake?.caseId
    if (caseId) {
      addLine('tool', `handoff ${res.status} caseId=${caseId}`)
    }
  } catch (error) {
    console.log('ROLE 1 HANDOFF failed')
  }
}
    ...
    ws.onmessage = ({ data }) => {
      ...
      switch (msg.type) {
        ...
        case 'transcript.user':
          ...
          addConversationTurn('patient', msg.text)
          logEvent('down', msg.type, msg.text)
          break
        ...
        case 'transcript.agent':
          ...
          addConversationTurn('agent', msg.text)
          logEvent('down', msg.type, msg.text)
          break
        ...
        case 'session.ended':
          logEvent('down', msg.type)
          console.log('FINAL CALL TRANSCRIPT:')
          console.log(JSON.stringify(conversationTurns, null, 2))
          submitHandoff(sessionId)
          ws.close()
          break
