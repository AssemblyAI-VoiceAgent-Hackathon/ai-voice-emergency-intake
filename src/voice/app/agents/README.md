...
```sh
cp app/agents/http-tools.jsonc app/agents/my-agent.jsonc
AGENT=my-agent python -m scripts.publish
```
...
For credentials, write `${MY_KEY}` anywhere in the file and put the value in `.env`, or in `app/agents/my-agent.env` if only this agent uses it. Both are gitignored. If a variable is missing, publishing stops and names it.
...
Tools with an `http` block are executed by AssemblyAI, so they work in a browser tab and on a phone call. Tools without one have to be answered by whatever holds the session, and a phone call has nothing to answer them. `python -m scripts.publish` prints a warning when a tool is missing it.
