...
`AI_Emergency_Intake_Team_Workflow_Roles_v4.pdf` is an exact versioned mirror of the current document. Team links should use the stable filename above so a future approved revision does not create competing “current” files.
...
- Publish the current document as `AI_Emergency_Intake_Team_Workflow_Roles.pdf` and keep the exact v4 mirror; preserve earlier or user-edited copies in `archive/`.
