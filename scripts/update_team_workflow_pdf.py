...
OUT = ROOT / 'output/pdf/AI_Emergency_Intake_Team_Workflow_Roles.pdf'
PREVIEW = ROOT / 'tmp/pdfs/workflow-v2-candidate.pdf'
...
def footer(c,page):
    ...
    c.drawRightString(W-42,24,f'Architecture Draft v2  |  31 Aug 2026  |  Page {page}')
...
for i,page in enumerate(reader.pages,1):
    ...
    if i==1:
        edits['A shared architecture proposal before implementation begins']='Updated architecture proposal for team review | v2 | 31 Aug 2026'
        edits['Decision gate: the team reviews and agrees on this structure before role-level implementation starts.']='Team review: confirm the updated handoffs and next actions on page 5. Final clinical decisions remain staff-owned.'
    if i==3:
        edits.update({
    ...
    if i==1:
        ...
        p(c,'31 AUG UPDATE - WHAT CHANGED SINCE v1',68,644,1047,12,color=WHITE,bold=True)
        p(c,'<b>Updated:</b> FastAPI + SSE is the current team direction. The live-event connection from Role 3 to Role 5 is now labelled SSE; staff review returns through the backend API.',68,665,1047,11,color=WHITE)
        p(c,'<b>Unchanged:</b> the same five roles, workflow steps, follow-up questions, record retrieval and human approval. <b>Still open:</b> final data format, database and each owner\'s implementation readiness.',68,699,1047,10.5,color=WHITE)
...
p(c,'31 August update - review the changes, confirm the open decisions, then connect one shared synthetic case.',42,58,1040,10.5,color=MUTED)
...
p(c,'Next deliverable from each owner',42,end+15,1100,12.5,bold=True,color=NAVY)
 ...
 ('<b>Role 1 - Voice AI</b>','Provide transcript turns and session references; connect the follow-up response from Role 2.','One synthetic spoken exchange becomes text, followed by one question and answer.'),
 ('<b>Role 2 - AI / Agent</b>','Align extraction fields with the sample; retain source labels; prepare the prompt and expected outputs.','Examples for sufficient, missing and conflicting information; output ready for Role 3 validation.'),
 ('<b>Role 3 - Backend</b>','Confirm intake/review interfaces and authentication; publish the agreed sample through SSE.','One sample update reaches Role 5; reconnect works; review and save confirmation are returned.'),
 ('<b>Role 4 - Data</b>','Confirm database, identifiers, authorised retrieval and approved-record/audit storage.','One synthetic lookup, one not-found result and one saved approved record without duplicate writes.'),
 ('<b>Role 5 - Frontend</b>','Display SSE draft updates; show gaps/conflicts; send staff edits and approval back to Role 3.','One live update without refresh; staff corrections survive later updates; save status is visible.'),
 ('<b>Demo - Maryam</b>','Coordinate a script and joint test once owners report readiness; use synthetic data only.','One complete intake-to-review-to-save demonstration; any simulated component is labelled.')
...
writer.add_metadata({
 ...
 '/Subject':'Architecture Draft v2 - 31 Aug 2026 - FastAPI SSE handoff update; same baseline layout',
 '/Keywords':'team workflow, role ownership, FastAPI, SSE, v2, team review',
})
